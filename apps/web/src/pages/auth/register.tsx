    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-gray-500 mt-2">{t('auth.completeRegistration')}</p>
        </div>

        <div className="bg-white rounded-xl p-8 shadow-sm border border-gray-100">
          <form onSubmit={form.handleSubmit(onSubmit)}>
            <h2 className="text-lg font-semibold text-gray-900 mb-6">{t('auth.yourDetails')}</h2>
            ...
            >
              {form.formState.isSubmitting ? t('common.saving') : t('auth.completeRegistration')}
            </button>
          </form>
        </div>
      </div>
